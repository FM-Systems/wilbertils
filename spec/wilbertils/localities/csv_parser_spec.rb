...
describe Wilbertils::Localities::CsvParser do
  ...
  describe '.parse' do
    ...
    it 'marks rows has_locality_type when a Category header is present' do
      result = subject.parse(io_for(<<~CSV), filename: filename)
        Postcode,Locality,State,Category,Longitude,Latitude
        4000,BRISBANE,QLD,LVR,153.0,-27.0
      CSV

      expect(result.rows.first.has_locality_type).to be(true)
    end

    # nil locality_type here means the file said nothing about types, not that
    # the locality is untyped - the distinction ChangeSet needs.
    it 'marks rows has_locality_type false when there is no Category header' do
      result = subject.parse(io_for(<<~CSV), filename: filename)
        Postcode,Locality,State,Longitude,Latitude
        4000,BRISBANE,QLD,153.0,-27.0
      CSV

      expect(result.rows.first.has_locality_type).to be(false)
      expect(result.rows.first.locality_type).to be_nil
    end

    it 'pads a stripped AU postcode back to 4 digits' do
      result = subject.parse(io_for(<<~CSV), filename: 'AU_auspost.csv')
        Postcode,Locality,State,Longitude,Latitude
        800,DARWIN,NT,130.8,-12.4
      CSV

      expect(result.rows.first.postcode).to eq('0800')
    end

    it 'pads a stripped NZ postcode back to 4 digits' do
      result = subject.parse(io_for(<<~CSV), filename: 'NZ_nzpost.csv')
        Postcode,Locality,State,Longitude,Latitude
        110,KAITAIA,NORTHLAND,173.2,-35.1
      CSV

      expect(result.rows.first.postcode).to eq('0110')
    end

    it 'leaves a 4-digit postcode unchanged' do
      result = subject.parse(io_for(<<~CSV), filename: 'AU_auspost.csv')
        Postcode,Locality,State,Longitude,Latitude
        3054,CARLTON NORTH,VIC,144.9,-37.7
      CSV

      expect(result.rows.first.postcode).to eq('3054')
    end

    it 'does not pad a postcode for a country outside the width map' do
      result = subject.parse(io_for(<<~CSV), filename: 'US_uspost.csv')
        Postcode,Locality,State,Longitude,Latitude
        123,SPRINGFIELD,IL,-89.6,39.8
      CSV

      expect(result.rows.first.postcode).to eq('123')
    end

    it 'does not pad a non-numeric postcode' do
      result = subject.parse(io_for(<<~CSV), filename: 'NZ_nzpost.csv')
        Postcode,Locality,State,Longitude,Latitude
        AB1,SOMEWHERE,REGION,173.2,-35.1
      CSV

      expect(result.rows.first.postcode).to eq('AB1')
    end
  end
  ...
  describe '.quick_validate' do
    ...
    it 'does not blow up on a short postcode that needs padding' do
      expect(subject.quick_validate(io_for("Postcode,Locality,Longitude,Latitude\n800,DARWIN,130.8,-12.4\n"), filename: 'AU_auspost.csv')).to be(true)
    end
  end
end
